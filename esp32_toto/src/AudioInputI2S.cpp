...
AudioInputI2S::AudioInputI2S(int port, int input_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
  this->dma_buf_count = dma_buf_count;
  if (input_mode != EXTERNAL_I2S && input_mode != INTERNAL_ADC) {
    input_mode = EXTERNAL_I2S;
  }
...
AudioInputI2S::AudioInputI2S(long sampleRate, pin_size_t sck, pin_size_t data) {
    i2sOn = false;
    mono = false;
    bps = 16;
    channels = 2;
...
bool AudioInputI2S::SetPins(int bclk, int wclk, int din)
{
  bclkPin = bclk;
  wclkPin = wclk;
  dinPin = din;
...
bool AudioInputI2S::SetChannels(int channels)
{
...
bool AudioInputI2S::SetOutputModeMono(bool mono)
{
  this->mono = mono;
  return true;
}

bool AudioInputI2S::SetLsbJustified(bool lsbJustified)
{
  this->lsb_justified = lsbJustified;
  return true;
}

bool AudioInputI2S::begin(bool rxADC)
{
  #ifdef ESP32
    if (!i2sOn)
    {
      ...
      if (input_mode == INTERNAL_ADC)
      {
        ...
        mode = (i2s_mode_t)(mode | I2S_MODE_ADC_BUILT_IN);
#else
        return false;      
#endif
      }
      ...
      if (channels == 1) {
          ...
          ch_fmt = I2S_CHANNEL_FMT_RIGHT_LEFT;
      }
#endif
    ...
    if (!i2sOn)
    {
      ...
      if (!i2s_rxtxdrive_begin(false, true, false, rxADC)) {
        return false;
      }
    #else
...
bool AudioInputI2S::ConsumeSample(int16_t sample[2])
{
...
int AudioInputI2S::read(int16_t *samples, int count) {
    size_t bytes_read = 0;
    i2s_read((i2s_port_t)portNo, samples, sizeof(int16_t) * count, &bytes_read, portMAX_DELAY);
